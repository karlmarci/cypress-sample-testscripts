...
describe('STIHL preg profile(negative)', function(){
    beforeEach(()=>{
        ...
        cy.fixture('dealer_nl').then((dealer_fr)=>{
            localStorage.setItem('local.app.v5.0.0_dealer', JSON.stringify(dealer_fr))
    ...
    it('Check Profile page translations', function(){
        cy.readFile('src/assets/i18n/FR_fr.json').then((json)=>{
            cy.get('[data-cy=profile-header]').then((header) => {
                var s1 = '<a href="http://www.stihl.fr/mentions-legales.aspx" target="_blank">'
                var s2 = "<a href='http://www.stihl.fr/mentions-legales.aspx' target='_blank'>"
                expect(header.html()).to.equal(json['profile_page_description_text'].replace(s2, s1))
              })
