...
describe('Home Module', () => 
{
 ...
 it('Selecting a Language (per Country)', function() 
 {
   ...
   cy.get('[data-cy=language-ddl]').select('Néerlandais').wait(1000)
 ...
 */
 ...
 it('View links', () =>
 {
   cy.get('.start-link').eq(0).click({force:true})
     .should('have.attr','href','https://nl.stihl.be/productregistratie.aspx')
   cy.wait(2000)
   cy.get('.start-link').eq(1).click({force:true})
     .should('have.attr','href','http://nl.stihl.be/wettelijke-vermeldingen-subpage.aspx')
 })
